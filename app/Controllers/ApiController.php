...
class ApiController extends Controller
{
    ...
    public function endpoint_browse( WP_REST_Request $request )
    {
        ...
        $cache_key = sprintf( 
            ...
            $args['keywords'],
            is_array( $args['types'] ) ? implode( '.', $args['types'] ) : 'na',
            $args['limit']
         );
        // Parse types
        ...
        // Get
        $results = get_theme_mod( 'superbrowse_cache', false ) ? Cache::remember( 
            $cache_key,
            get_theme_mod( 'superbrowse_cache_minutes', 120 ),
            function () use( &$args ) {
                return Superbrowse::browse( $args );
            }
         ) : Superbrowse::browse( $args );
        // Response
        $response = new WP_REST_Response( array_map( function ( $result ) {
